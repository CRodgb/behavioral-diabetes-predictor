...
def build_model_input(ui_values: dict) -> pd.DataFrame:
    ...
    for col in train_df.columns:
        if col in ui_values:
            value_to_save = ui_values[col]

            if col == "Body_Mass_Index":
                value_to_save = value_to_save * 100.0

            row[col] = value_to_save
        else:
...
def build_report_text(
    ...
    lines.append("")
    lines.append("Note: This report is generated by a machine learning model")
    lines.append("trained on BRFSS 2024 data and is for educational/demo")
    lines.append("purposes only. It is not a medical diagnosis.")
